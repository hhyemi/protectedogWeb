				...
				<table class="mdl-data-table mdl-js-data-table mdl-shadow--4dp">
					<thead>
						<tr align="center">
							...
							<th width="20%" align="center" class="text-center">신고일시</th>
							<th width="5%">처리상태</th>
							<th width="5%">더보기</th>				
						</tr>
					</thead>
					...
					<c:set var="i" value="0" />
						<c:forEach var="report" items="${list}">
							<tr>
								<td align="center">${ report.reporterId }</td>
								<td align="center">${ report.reportedId }</td>
								<td align="center" class="text-center">${ report.reportCategory }</td>
								<td align="center">
									<fmt:formatDate value="${ report.reportDate }" pattern="yyyy년 MM월 dd일"/>
								</td>
								<c:if test="${ report.reportStatus == 0 }">
									<td align="center">(${ report.reportStatus })처리중</td>
								</c:if>
								<c:if test="${ report.reportStatus == 1 }">
									<td align="center">(${ report.reportStatus })처리완료</td>
								</c:if>
								<td align="center">
...
<!-- 						</div> -->
	...
	$(function(){
		$("td:nth-child(6)").on("click", function(){
			var reportNo=$(this).children("input").val();
			alert(reportNo);
